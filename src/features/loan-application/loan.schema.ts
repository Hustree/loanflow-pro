...
export const loanTypeEnum = z
  .string()
  .refine((val): val is LoanTypeEnum => ['Emergency', 'Salary', 'Others'].includes(val), {
    message: 'Must be Emergency, Salary, or Others',
  });

export const loanTermEnum = z
  .number()
  .refine((val): val is LoanTermEnum => [12, 24, 36, 48].includes(val), {
    message: 'Term must be 12, 24, 36, or 48 months',
  });

export const disbursementModeEnum = z
  .string()
  .refine((val): val is DisbursementModeEnum => ['PNB', 'UnionBank', 'Cebuana'].includes(val), {
    message: 'Must be PNB, UnionBank, or Cebuana',
  });

export const loanStatusEnum = z
  .string()
  .refine(
    (val): val is LoanStatusEnum =>
...
export const loanSchema = z.object({
  ...
  pnpBfpId: z
    .string()
    .min(6, 'PNP/BFP ID must be at least 6 characters')
    .max(12, 'PNP/BFP ID must be at most 12 characters'),
  amount: z
    .number()
...
export const statusUpdateSchema = z.object({
  ...
  notes: z
    .string()
    .min(1, 'Notes are required for status updates')
